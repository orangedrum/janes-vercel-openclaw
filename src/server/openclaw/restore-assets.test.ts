...
test("buildRestoreRuntimeEnv includes gateway token and base URL only", () => {
  ...
  // AI Gateway key is injected via network policy transform, not env vars.
...
test("buildBootstrapFiles includes static, dynamic, token, and manifest files", () => {
  ...
  // AI gateway key is NOT written — injected via network policy transform
